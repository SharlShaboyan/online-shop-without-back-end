        ...
        if (nameA && reg.test(phoneA) && streetA && homeA && selectedValue && basketItems.length > 0) {
            try {
                await axios.post(URI_API, {
                    ...
                    text: {
                        "Сайт" : "дорогой",
                        "заказчик": `<b>Имя:</b> ${nameA}     <b>Телефон:</b> +7${phoneA}    <b>Город:</b> ${selectedCity}    <b>Улица:</b> ${streetA}     <b>Дом:</b> ${homeA}    <b>Дополнительно:</b> ${otherA ? otherA : "---"}    <b>Стоимость доставки:</b> ${selectedValue}    <b>итого:</b> ${totalPrice}     `,
                        "заказ": sendItems()
                    }
                })
